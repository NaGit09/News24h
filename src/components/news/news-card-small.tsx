...
export function NewsCardSmall({title, sapo, image, href, publishedAt}: NewsCardSmallProps) {
    const [imgError, setImgError] = useState(false)

    return (
        <Link to={href} className="group">
            <article
                className="flex gap-3 py-3 border-b border-border/50 last:border-0 transition-all hover:bg-accent/5">
                ...
                <div className="flex-1">
                    ...
                    {sapo && <p className="mb-1 text-xs leading-relaxed text-muted-foreground line-clamp-2">{sapo}</p>}
